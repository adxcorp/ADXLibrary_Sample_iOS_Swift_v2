...
class NativeAdPlacerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let sizeHandler: ADXNativeViewSizeHandler = { maximumWidth in
            return CGSize(width: maximumWidth, height: 300.0)
        }

        self.placer = ADXNativeAdFactory.sharedInstance().getCollectionViewAdPlacer(
            ADX_NATIVE_AD_UNIT_ID,
...
extension NativeAdPlacerViewController: UICollectionViewDataSource {
    ...
    {
        let cell : UICollectionViewCell = collectionView.adx_dequeueReusableCell(withReuseIdentifier: "CustomCell", 
